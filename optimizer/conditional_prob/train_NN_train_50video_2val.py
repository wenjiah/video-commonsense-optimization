...
training_data = CustomDatasetForTrain(train_dir=train_dir, dataset=val_dataset[:int(0.8*len(val_dataset))], te=te, target_key=-1, train_cond_prob_dic=train_cond_prob_dic)
val_data = CustomDatasetForTrain(train_dir=train_dir, dataset=val_dataset[int(0.8*len(val_dataset)):], te=te, target_key=-1, train_cond_prob_dic=train_cond_prob_dic)
...
for epoch in range(n_epochs):
    ...
    if (epoch+1)%eval_every == 0:
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), '../../temp_result/training_model_50video_2val.pt')
