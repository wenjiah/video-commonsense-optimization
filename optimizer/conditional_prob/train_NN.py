...
# from train_prob import train_prob
# from conditional_prob.train_prob import train_prob

class CustomDatasetForTrain(Dataset):
    def __init__(self, train_dir, dataset, te, target_key, train_cond_prob_dic=None):
        self.feature = []
        self.label = []
        if target_key == -1:
            for cur_tuple in dataset:
                ...
                for target_obj in target_candidates:
                    ...
                    for obj in cur_tuple:
                        if obj != target_obj:
                            cur_feature.append(np.array([train_cond_prob_dic[target_obj][obj]])) # len=1
            ...
            for cur_tuple in dataset:
                ...
                for obj in cur_tuple:
                    if obj != te.columns_[target_key]:
                        # cur_feature.append(np.array([train_cond_prob[obj]]))
                        cur_feature.append(np.array([train_cond_prob_dic[te.columns_[target_key]][obj]]))
                self.feature.append(torch.Tensor(cur_feature))
