...
class CustomDatasetForConcept(Dataset):
    def __init__(self, dataset, te, target_key, numberbatch_model):
        self.feature = []
        self.label = []
        if target_key == -1:
                    ...
                    for obj in cur_tuple:
                        if obj != target_obj:
                            if target_obj.replace(" ", "_") in numberbatch_model and obj.replace(" ", "_") in numberbatch_model:
                                cur_feature.append(np.array([numberbatch_model.similarity(target_obj.replace(" ", "_"), obj.replace(" ", "_"))])) # len=1
                            else:
                                cur_feature.append(np.array([0])) # len=1
                ...
                for obj in cur_tuple:
                    if obj != te.columns_[target_key]:
                        if te.columns_[target_key].replace(" ", "_") in numberbatch_model and obj.replace(" ", "_") in numberbatch_model:
                            cur_feature.append(np.array([numberbatch_model.similarity(te.columns_[target_key].replace(" ", "_"), obj.replace(" ", "_"))]))
                        else:
                            cur_feature.append(np.array([0]))
                self.feature.append(torch.Tensor(cur_feature))
